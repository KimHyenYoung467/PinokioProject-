...
namespace Sprite.UGUI.UIscript
{
    public class Inventory : InventoryManager
    {
        ...
        private string ItemDscription(string itemDescription)
        {
            ...
            if (Rect.gameObject.CompareTag("Inventory"))
                StatusText.text = itemDescription;
            return StatusText.text;
        }

        //Todo 클릭 위치에 따라서 인벤토리 테이블을 선택하는 SelectBar 이동 
        private Vector3 MoveSelectBar(RectTransform _selectBar, GameObject selectObj)
        {
            //Todo 현재 선택바 가 어디에 위치해 있는 지 확인.
            ...
            {
                _selectBar.transform.transform.position = Input.mousePosition; // 선태바의 위치를 마우스 포인터로 선택한 아이템 테이블 위치로 이동. 
        ...
        private void Imgprint(Image itemChangeImg, GameObject selectObj, ItemUnitInfo itemUnit)
        {
            ...
            else if (!selectBar.gameObject.transform.position.Equals(Input.mousePosition) || !itemUnit.ItemID.Equals(itemUnit.ItemID));
            {
